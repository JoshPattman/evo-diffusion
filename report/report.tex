...
\begin{document}
    ...
    Mutations on this model are applied with a different process to that of the original. Firtly, the matrix to mutate is randomly chosen with a probability which heavily favours the matrix $B_m$. In the following experiments, the chance of mutating $B_s$ was $0.1\%$. If $B_m$ was chose, mutations are applied as descibed in the original model. However, if $B_s$ is chose to be mutated, a random cell of the matrix is changed to a value chosen unformly from the set $\{-1,0,1\}$, irrespective of what the cell's value was before.

    \section{Extension Results}
    \begin{itemize}
        \item Repeat Exp2 but with the new mutations. Specifically, study the genes over epochs graph and and weights matrix
        \item Create a metric for unique phenotypes prduced, part of the genralised set / not
        \item Using new dataset, compare number of unique training phenotypes produced over generations with dense and maskeddense, and also l2dense
        \item Show an image of the produced Bestiary
    \end{itemize}
